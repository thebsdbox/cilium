...
The flag defaults to ``default=8``, which means it will always pre-allocate at least
8 IPs from the ``default`` pool. All other pools which do not have an entry in the
``ipam-multi-pool-pre-allocation`` map are assumed to have a ``preAllocIPs`` of zero,
i.e. no IPs are pre-allocated for that pool.
